...
export async function POST(req: Request) {
    try {
        ...
        const { RequestType,
            RequestNo,
            LineNo, } = body;
        const result = await purchaseRequisitionService.DeleteRequestLine(RequestType,
            RequestNo,
            LineNo,);
        ...
        return NextResponse.json(
            { error: err?.response?.data?.error?.message || "Failed to delete request line", },
